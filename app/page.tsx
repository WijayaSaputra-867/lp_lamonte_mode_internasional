  ...
  return (
    <div className='relative isolate overflow-hidden bg-background'>
      ...
      <nav className='fixed top-0 z-50 w-full border-b border-zinc-100 bg-white/80 backdrop-blur-md'>
        ...
        {/* Mobile Nav Overlay */}
        <div
          className={`absolute top-full left-0 w-full border-b border-zinc-100 bg-white transition-all duration-300 md:hidden ${isMenuOpen ? "opacity-100 translate-y-0 visible" : "opacity-0 -translate-y-4 invisible"}`}>
        ...
        <div className='mx-auto max-w-7xl px-6'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-8'>
            {featuresToRender.map((feature, i) => {
              ...
              // Render fitur standar (Modul A-F)
              return (
                <div
                  key={feature.id}
          ...
          {isMobile && (
            <div className='mt-8 flex justify-center md:hidden'>
              <button
                ...
                {visibleCount >= shuffledFeatures.length ? (
                  <>
          ...
          {(() => {
            ...
            return (
              <>
                ...
                {/* Tombol Khusus Mobile: Link ke WA untuk lihat katalog lengkap (pengganti pagination) */}
                {isMobile && (
              ...
              {allTiers
                ...
                .map((tier, i) => (
                  <div
                    ...
                    <h3 className='text-lg md:text-xl font-bold text-zinc-900 mb-1 outfit'>
                      {tier.name}
                    </h3>
                    <p className='text-[10px] md:text-xs text-zinc-400 mb-4 font-bold uppercase tracking-widest'>
                      {tier.desc}
                    </p>
